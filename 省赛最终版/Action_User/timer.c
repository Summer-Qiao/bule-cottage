...
void TIM4_Pwm_Init(u32 arr,u32 psc)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE); //使能 PORTB 时钟

	GPIO_PinAFConfig(GPIOD,GPIO_PinSource12,GPIO_AF_TIM4); //PB7 复用为 TIM4
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource14,GPIO_AF_TIM4); //PB7 复用为 TIM4
...
void TIM4_ROBS_Init(u32 arr,u32 psc)
{
		GPIO_InitTypeDef GPIO_InitStructure;
		TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
		TIM_OCInitTypeDef TIM_OCInitStructure;
	  ...
	  GPIO_PinAFConfig(GPIOB,GPIO_PinSource6,GPIO_AF_TIM4); //PB7 复用为 TIM4
	
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6; //GPIOB5
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF; //复用功能
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz; //速度 50MHz
		GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; //推挽复用输出
		GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; //上拉
...
void WindCtr(float percent)
{
	float val;
	val=0.06+percent/100.0*0.04;
	TIM_SetCompare2(TIM3,val*2000);
}
