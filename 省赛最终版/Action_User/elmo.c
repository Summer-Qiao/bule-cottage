...
void velCrl(uint8_t DriverNum,int velData)
{
	...
	vel.Data32 = velData;
	TxMessage.StdId= 0x280+DriverNum;					 // standard identifier=0
	TxMessage.ExtId= 0x280+DriverNum;					 // extended identifier=StdId
	TxMessage.IDE=CAN_Id_Standard;			 // type of identifier for the message is Standard
	TxMessage.RTR=CAN_RTR_Data;			 // the type of frame for the message that will be transmitted
	TxMessage.DLC=8;
...
void xp_velcrl(int baseV,int errV)
{

		VelCrl(1, baseV+errV);//errV为正，往左拐
		VelCrl(2,-(baseV-errV));
